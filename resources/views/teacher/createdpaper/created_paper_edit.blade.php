        ...
        <div class="container-full">
            ...
            <section class="content">
                <div class="row">
                    ...
                    <div class="col-12">
                        <form action="{{ route('update.quiz', $questions->id) }}" method="post">
                            @csrf
                            ...
                            <div class="box">
                                ...
                                <div class="row m-0 d-flex justify-content-between">
                                    <div class="col-md-3">
                                        ...
                                        <input class="form-control" type="date" name="date" value="{{ old('date',$questions->date) }}">
                                        <span class="text-danger">@error('date'){{ $message }}@enderror</span>
                                    </div>
                                    <div class="col-md-2">
                                        <label for="#msqstotal">Quiz Start Time</label>
                                        <input class="form-control" type="time" name="start_time" value="{{ old('time',$questions->start_time) }}">
                                        <span class="text-danger">@error('start_time'){{ $message }}@enderror</span>
                                    </div>
                                    <div class="col-md-2">
                                        <label for="#msqstotal">Quiz End Time</label>
                                        <input class="form-control" type="time" name="end_time" value="{{ old('time',$questions->end_time) }}">
                                ...
                                @php
                                    if (old('select_m')){
                                        $select_old_m = old('select_m');
                                    }elseif(!empty($find_questions)){
                                        $select_old_m = $find_questions;
                                    }else{
                                        $select_old_m = array();
                                    }
                                    if (old('select_t')){
                                        $select_old_t = old('select_t');
                                    }elseif(!empty($find_questions)){
                                        $select_old_t = $find_questions;
                                    }else{
                                        $select_old_t = array();
                                    }
                                @endphp
                                    ...
                                    <div class="table-responsive">
                                        <table id="example1" class="table table-bordered table-striped">
                                            ...
                                            @foreach($subjective as $key => $sub)
                                                <tr>
                                                    <td>
                                                        <input type="checkbox" id="basic_checkbox2_{{$key}}" name="select_t[]"  value="{{$sub->id}}" {{ in_array($sub->id,$select_old_t) ? 'checked' : '' }}>
